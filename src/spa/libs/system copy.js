...
(function() {
  ...
  function resolveAndComposePackages(
    ...
    for (let p in packages) {
      ...
      if (!mapped) targetWarning(p, rhs, "bare specifier did not resolve");
    ...
    systemJSPrototype.instantiate = function(url, parent) {
      ...
      switch (ext) {
        ...
        case ".html":
          return getSourceRes().then(function(res) {
            return (
              maybeJSFallback(res) ||
              loadError("'.html' modules not implemented")
            );
          });
        case ".json":
          return loadDynamicModule(function(_export, source) {
      ...
      function getSourceRes() {
        return fetch(url).then(function(res) {
          if (!res.ok) loadError(res.status + " " + res.statusText);
          return res;
        });
      }
      ...
      function loadError(msg) {
        throw Error(
          msg + ", loading " + url + (parent ? " from " + parent : "")
        );
      }
    };
  })(typeof self !== "undefined" ? self : global);
