...
(function() {
  ...
  function resolvePackages(pkgs, baseUrl) {
    ...
    for (var p in pkgs) {
      ...
      outPkgs[resolveIfNotPlainOrUrl(p, baseUrl) || p] = resolveUrl(
        value,
        baseUrl
      );
    }
    ...
    if (json.scopes) {
      for (let scopeName in json.scopes) {
        ...
        scopes[resolvedScopeName] =
          resolvePackages(scope, resolvedScopeName) || {};
      }
    }
  ...
  function resolveImportMap(id, parentUrl, importMap) {
    const urlResolved =
      resolveIfNotPlainOrUrl(id, parentUrl) || (id.indexOf(":") !== -1 && id);
    ...
    return (
      applyPackages(id, importMap.imports) ||
      urlResolved ||
      throwBare(id, parentUrl)
    );
  }
        ...
        return [
          ...
          function(_export) {
            return {
              setters: setters,
              execute: function() {
                return WebAssembly.instantiate(module, importObj).then(function(
                  instance
                ) {
  ...
  if (acquiringImportMaps) {
    Array.prototype.forEach.call(
      document.querySelectorAll('script[type="systemjs-importmap"][src]'),
      function(script) {
        ...
        function(script) {
          importMapPromise = importMapPromise.then(function(map) {
            return (
              ...
              return mergeImportMap(
                map,
                parseImportMap(json, script.src || baseUrl)
              );
            });
          });
        }
